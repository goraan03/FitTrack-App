...
function StatusBadge({ status }: { status: BillingStatus['billing_status'] }) {
  const map = {
    trial:    { label: "TRIAL",    cls: "bg-amber-400/10 text-amber-400 border-amber-400/20" },
    active:   { label: "ACTIVE",   cls: "bg-emerald-400/10 text-emerald-400 border-emerald-400/20" },
    past_due: { label: "PAST DUE", cls: "bg-orange-400/10 text-orange-400 border-orange-400/20" },
    suspended:{ label: "SUSPENDED",cls: "bg-red-400/10 text-red-400 border-red-400/20" },
    none:     { label: "INACTIVE", cls: "bg-slate-400/10 text-slate-400 border-slate-400/20" },
  };
...
function TrialBanner({ billing }: { billing: BillingStatus }) {
  ...
  return (
    <div className={`rounded-2xl p-5 border flex items-start gap-4 mb-8 ${
      urgent
        ? "bg-red-400/5 border-red-400/20"
        : "bg-amber-400/5 border-amber-400/20"
    }`}>
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
        urgent ? "bg-red-400/10" : "bg-amber-400/10"
      }`}>
      ...
      <div>
        <p className={`font-bold text-sm uppercase tracking-wider mb-1 ${urgent ? "text-red-400" : "text-amber-400"}`}>
          {urgent ? `Trial ističe za ${daysLeft} ${daysLeft === 1 ? "dan" : "dana"}!` : `Trial period — još ${daysLeft} dana`}
        </p>
        <p className="text-slate-400 text-sm">
          {urgent
            ? "Odaberi paket odmah da izbjegneš prekid usluge."
            : "Isprobaj sve funkcionalnosti besplatno. Odaberi paket kada budeš spreman."}
        </p>
      </div>
    </div>
  );
...
function PlanCard({
  ...
  if (isCurrent) {
    actionLabel = "TRENUTNI PAKET";
    actionDisabled = true;
  } else if (noCurrentPlan) {
    action = 'select';
    actionLabel = tooSmall ? `PREVIŠE KLIJENATA (max ${plan.max_clients})` : "ODABERI PAKET";
    actionDisabled = tooSmall;
  } else if (current && plan.tier > current.tier) {
    action = 'upgrade';
    actionLabel = hasPending && pending!.id === plan.id
        ? "UPGRADE (zakazano)"
        : "UPGRADE (sljedeći ciklus)";
    actionDisabled = hasPending && pending?.id !== plan.id; // blokira dok ima pending
  } else if (current && plan.tier < current.tier) {
    action = 'downgrade';
    actionLabel = tooSmall
        ? `PREVIŠE KLIJENATA (max ${plan.max_clients})`
        : hasPending && pending!.id === plan.id
        ? "DOWNGRADE (zakazano)"
        : hasPending
            ? "VEĆ IMA ZAKAZAN PLAN"        // blokira ostale
            : "DOWNGRADE (sljedeći ciklus)";
    actionDisabled = tooSmall || (hasPending && pending?.id !== plan.id);
    }

  return (
    <div className={`relative bg-[#111118] border rounded-2xl p-6 flex flex-col gap-5 transition-all duration-300 ${
      isCurrent
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-slate-500 text-xs uppercase tracking-widest mt-1">
            do {plan.max_clients === 9999 ? "∞" : plan.max_clients} klijenata
          </p>
        </div>
      ...
      <div>
        ...
        <span className="text-slate-500 text-sm ml-1">EUR / mj.</span>
      </div>
...
export default function TrainerBillingPage({ trainerApi }: Props) {
  ...
  const load = async () => {
    ...
    try {
      ...
      toast.error("Greška pri učitavanju billing podataka");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="pb-12">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-10 opacity-0 animate-fade-in-up" style={{ animationFillMode: "forwards" }}>
          <div>
            <h1 className="text-3xl lg:text-4xl font-bold text-white mb-2">
              BILLING &amp; <span className="text-amber-400">PAKETI</span>
            </h1>
            <p className="text-slate-400 text-sm tracking-wide uppercase">
              Upravljaj pretplatom i limitima
            </p>
          </div>
        ...
        {billing?.current_plan && (
          <div className="bg-[#111118] border border-[#27273a] rounded-2xl p-6 mb-8 opacity-0 animate-fade-in-up stagger-1" style={{ animationFillMode: "forwards" }}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-lg font-bold text-white uppercase tracking-wider">Pregled pretplate</h2>
            </div>
            ...
            {billing.pending_plan && (
              <div className="mt-4 flex items-center gap-3 p-4 bg-orange-400/5 border border-orange-400/15 rounded-xl">
                ...
                <p className="text-sm text-orange-300">
                  Downgrade na <strong>{billing.pending_plan.name}</strong> zakazan za sljedeći billing ciklus.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="mb-4 opacity-0 animate-fade-in-up stagger-2" style={{ animationFillMode: "forwards" }}>
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-xl font-bold text-white uppercase tracking-wider">Dostupni paketi</h2>
          </div>
