  ...
  return (
    <section className="space-y-10 pb-20 px-4 sm:px-6 max-w-7xl mx-auto opacity-0 animate-fade-in-up">
        ...
        <div className="relative z-10 w-full px-6 py-16 sm:px-12 text-center flex flex-col items-center">
          ...
          <div className="inline-flex items-center gap-2 rounded-full border border-amber-400/20 bg-amber-400/5 px-4 py-1.5 mb-8 backdrop-blur-md">
            ...
            <span className="text-[9px] font-black uppercase tracking-[0.2em] text-amber-400/80">
              {t('landing_badge') || "Training Management Platform"}
            </span>
          </div>

          {/* Glavni Naslov */}
          <h1 className="max-w-4xl text-4xl sm:text-6xl font-black tracking-tighter text-white leading-[1.1] uppercase italic">
            {(t('fittrack_dashboard') || "FITTRACK DASHBOARD").replace(' DASHBOARD', '')} <span className="text-amber-400">{"DASHBOARD"}</span>
          </h1>

          <h2 className="mt-4 max-w-2xl text-lg sm:text-xl font-bold text-slate-300 tracking-tight">
            {t('landing_subtitle') || "Stay aligned — sessions, programs, and progress in one place."}
          </h2>

          {/* Opis */}
          <p className="mt-6 max-w-xl text-slate-500 text-sm sm:text-base leading-relaxed font-medium">
            {t('landing_desc') || "A clean workflow for scheduling, program delivery, and tracking performance — built for real training businesses."}
          </p>
          ...
          <div className="mt-10 flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
            ...
            >
              ...
              {t('create_client_account') || "Create Client Account"}
            ...
            >
              ...
              {t('trainer_access') || "Trainer Access"}
            </Link>
          </div>
          ...
          <div className="mt-8 flex items-center gap-2 text-[10px] text-slate-600 font-bold uppercase tracking-widest">
            ...
            {t('admin_note') || "Official Trainer accounts are managed by Admin"}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 opacity-0 animate-fade-in-up" style={{ animationDelay: "120ms", animationFillMode: "forwards" }}>
        <MiniStat icon={<Activity className="w-5 h-5 text-amber-400" />} label={t('sessions_label') || "Sessions"} value={t('sessions_val') || "Scheduling & capacity"} color="border-amber-400/20" />
        <MiniStat icon={<Users className="w-5 h-5 text-cyan-400" />} label={t('clients_label') || "Clients"} value={t('clients_val') || "Progress & history"} color="border-cyan-400/20" />
        <MiniStat icon={<BarChart3 className="w-5 h-5 text-violet-400" />} label={t('insights_label') || "Insights"} value={t('insights_val') || "Trends & feedback"} color="border-violet-400/20" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 opacity-0 animate-fade-in-up" style={{ animationDelay: "220ms", animationFillMode: "forwards" }}>
        <FeatureCard
          ...
          title={t('feat_scheduling_title') || "Scheduling that stays organized"}
          text={t('feat_scheduling_desc') || "Set available sessions, manage capacity, and keep bookings under control — everything stays in one calendar flow."}
        />
        <FeatureCard
          icon={<Users className="h-6 w-6" />}
          title={t('feat_programs_title') || "Programs clients actually follow"}
          text={t('feat_programs_desc') || "Create structured programs, assign them in seconds, and keep clients consistent with clear workout guidance."}
        />
        <FeatureCard
          icon={<Star className="h-6 w-6" />}
          title={t('feat_pro_title') || "Professional experience"}
          text={t('feat_pro_desc') || "A clean, modern look that makes your service feel premium and trustworthy — from day one."}
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 opacity-0 animate-fade-in-up" style={{ animationDelay: "320ms", animationFillMode: "forwards" }}>
        <FeatureCard
          ...
          title={t('feat_workouts_title') || "Clear workouts, better adherence"}
          text={t('feat_workouts_desc') || "Exercise details are easy to follow, so clients know exactly what to do and you spend less time explaining basics."}
        />
        <FeatureCard
          icon={<BarChart3 className="h-6 w-6" />}
          title={t('feat_progress_title') || "Progress & ratings"}
          text={t('feat_progress_desc') || "See trends over time, collect feedback, and make coaching decisions based on real signals — not guesswork."}
        />
      </div>
    </section>
  );
