  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setError(res.data.message || t('error_sending_msg') || "An error occurred while sending the message.");
      }
    } catch (err) {
      if (isAxiosError(err)) {
        setError(err.response?.data?.message || t('error_sending_msg') || "An error occurred while sending the message.");
      } else {
        setError(t('error_sending_msg') || "An error occurred while sending the message.");
      }
    } finally {
      setLoading(false);
    }
        ...
        <div className="relative z-10 text-center flex flex-col items-center">
          <div className="inline-flex items-center gap-2 rounded-full border border-amber-400/20 bg-amber-400/5 px-4 py-1.5 mb-6">
            ...
            <span className="text-[9px] font-black uppercase tracking-[0.2em] text-amber-400/80">
              {t('contact_badge') || "Inquiry & Trainer Access"}
            </span>
          </div>

          <h1 className="text-4xl sm:text-6xl font-black tracking-tighter text-white leading-tight uppercase italic">
            {(t('get_in_touch') || "GET IN TOUCH").replace(' IN TOUCH', '')} <span className="text-amber-400">{"IN TOUCH"}</span>
          </h1>

          <p className="mt-6 max-w-2xl text-slate-400 text-sm sm:text-base leading-relaxed font-medium" dangerouslySetInnerHTML={{ __html: t('contact_desc') || "Have a question, feedback, or want to request a Trainer Account? Fill out the form below to contact the administrator and we'll get back to you shortly." }} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="flex items-center gap-3 mb-2 ml-2">
            ...
            <h2 className="text-xs font-black uppercase tracking-widest text-white italic">{t('information') || "Information"}</h2>
          </div>

          <InfoCard
            title={t('trainer_onboarding') || "Trainer Onboarding"}
            desc={t('trainer_onboarding_desc') || "Official trainer accounts are manually created by admins to ensure quality."}
            icon={<ShieldCheck className="w-5 h-5 text-amber-400" />}
            borderColor="border-amber-400/20"
          />
          <InfoCard
            title={t('customization') || "Customization"}
            desc={t('customization_desc') || "Want to adapt FitTrack to your specific gym or studio needs?"}
            ...
            <form onSubmit={submit} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 ml-1">{t('full_name') || "Full Name"}</label>
                  <input
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 ml-1">{t('email_address') || "Email Address"}</label>
                  <input
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 ml-1">{t('your_message') || "Your Message"}</label>
                <textarea
                  placeholder={t('msg_placeholder') || "Tell us about your request or gym studio..."}
              ...
              {sent && (
                <div className="text-[10px] font-bold text-emerald-400 bg-emerald-500/5 border border-emerald-500/10 rounded-xl px-4 py-3 uppercase tracking-widest">
                  {t('msg_sent') || "Your message has been sent. Thank you!"}
                </div>
              )}
