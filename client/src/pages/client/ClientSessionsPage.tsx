  ...
  const book = async (id: number) => {
    try {
      ...
      if (r.success) {
        ...
        toast.error(r.message || t('booking_failed'));
      }
    } catch (err: any) {
        ...
        >
          <div className="flex flex-col lg:flex-row lg:items-center gap-3 lg:gap-4">
            <div className="flex items-center gap-2 text-slate-300 font-semibold">
              ...
              <span className="uppercase tracking-wider text-xs">{t('filter_search')}</span>
            </div>
            ...
            <div className="flex-1 flex flex-col sm:flex-row gap-3">
              ...
              >
                <option value="">{t('all_sessions')}</option>
        ...
        <div className="space-y-4 opacity-0 animate-fade-in-up stagger-2" style={{ animationFillMode: "forwards" }}>
          {loading ? (
            <div className="bg-[#111118] border border-[#27273a] rounded-2xl p-8 flex items-center gap-4">
              ...
              <div className="text-slate-400 text-sm font-semibold uppercase tracking-widest">{t('loading')}...</div>
            </div>
          ) : visibleList.length === 0 ? (
            <div className="bg-[#111118] border border-[#27273a] rounded-2xl p-12 text-center">
              <div className="text-6xl mb-3">📅</div>
              <div className="text-slate-300 font-black uppercase tracking-widest text-sm">{t('no_sessions_found_msg')}</div>
              <div className="text-slate-500 text-sm mt-2">{t('try_adjusting_filters')}</div>
            </div>
          ) : (
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div className="min-w-0">
                      ...
                      <div className="text-sm text-slate-400">
                        ...
                        <span className="uppercase tracking-wider">{t(item.type)}</span> •{" "}
                    ...
                    <div className="flex items-center justify-end gap-2">
                      {item.isEnrolled ? (
                        isCancellable(item.startAt) ? (
                          ...
                          "
                          >
                            {bookingId === item.id ? `${t('canceling')}...` : t('cancel')}
                          </button>
                        ) : (
                        ...
                        "
                        >
                          {bookingId === item.id ? `${t('booking')}...` : t('book')}
                        </button>
                      ) : (
